...
- **Cuentas** — implementado: lista maestro-detalle (click en una cuenta muestra su detalle), formulario de nuevo movimiento (el tipo `uso` solo aparece para cuentas `cupo_revolvente`, igual que la regla del backend), formulario de actualización de cupo transaccional (solo `cupo_revolvente`), y tabla de movimientos. Para cuentas `fondo_fijo` (Guayaquil, Bolivariano, Pacífico, Fullcarga) hay una sección "Cuadre del fondo": muestra el valor inicial guardado y un solo campo "Valor actual" — lo que Joseph decide que queda en la cuenta — con un botón "Guardar cuadre" que calcula el recaudado y deja ese valor como el inicial del próximo cuadre, sin pasos separados de abrir/cerrar día.
