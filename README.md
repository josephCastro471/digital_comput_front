...
- **Cuentas** — implementado: lista maestro-detalle (click en una cuenta muestra su detalle), formulario de nuevo movimiento (el tipo `uso` solo aparece para cuentas `cupo_revolvente`, igual que la regla del backend), formulario de actualización de cupo transaccional (solo `cupo_revolvente`), y tabla de movimientos. Para cuentas `fondo_fijo` (Guayaquil, Bolivariano, Pacífico, Fullcarga) hay una sección "Cuadre del fondo": un campo para el saldo al iniciar el día y otro para el saldo al cerrar/cuadrar — el sistema calcula y muestra lo recaudado (`saldo_inicial_dia - saldo_final`), replicando el flujo que antes se hacía a mano en Excel.
